...
```{r}
# for reproducible results we can set the seed
set.seed(6)
...
* for loop used to investigate repeated random samling and behavior of index.
```{r}
S.true <- length(jelly.jar)
...
for ( i in 1:100){
  sample.dj <- sample.community(jelly.jar,N)
  x<-S.obs(t(sample.dj))
