...
* We want to generate source comunity
* R has built-in functions for statistical distributions (see`Distributions')
* for each distribution there are 4 functions: density, probability, qunatile and random sample.
* Biological communities often described by log-normal so we will use that distribution.
* random are not really random, rather pseudo-random. We can get the same "random" sample by setting the seed
...
* for loop used to investigate repeatedd random samling and behavior of index.
```{r}
