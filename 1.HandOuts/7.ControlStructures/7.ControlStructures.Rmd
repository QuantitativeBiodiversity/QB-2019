...
Often, control structures are embedded in functions to cary out sequential, selective, and repetitive tasks. 
Some of the most commonly used control structures include "if, else" statements, "while" loops, and "next" statments. 
Today, we will focus mostly on the "for loop".

There is a lot of information on control structures elsewhere. 
A succint tutorial of common control structures in R can be found at https://ramnathv.github.io/pycon2014-r/learn/controls.html
...
### B. For-Loops
These control structures execute an interative task. 
...
In the above for-loop sturcture, i is a variable that will hold each interger from 1 to 10 during each successive pass through the loop.  
...
For example, the function `tapply()` executes a function to sepcific elements of a dataframe, `apply()` executes a function (e.g., mean) to the rows or columns of a data frame, and `sapply()` peforms a function on list or array.  
...
While loops are sort of like repeating "if" statement; that is, it continues to operate until a conditional statment changes (e.g., "true" becomes "false"). 
...
The control structures discussed above have unlimited applications in the realm of quatitative methods. 
...
But before we can do that we need to get familiar with another powerful family of functions in R: built-in probability ditributions.  

### A. Statistical Distributions 

Many groups of quantities can be described as frequency distributions. 
You are probably familiar with the Normal ditribution and its hallmark bell curve. 
...
You will find that each disribution has four functions asociated with it: **d**ensity, **p**robability, **q**uantiles and **r**andom sample. 
For example the uniform distribution (an equal probaility for all values) is called *unif* in R and has the following four functions.     
...
Computers can't generate truly random number, but rather pseudorandom numbers. 
...
This function iterated over the samlping step `n` times. 
...
But pass some inapropriate argument and things will go wrong, and posssibly horribly wrong. 
To demostrate this idea try and sample the `jelly.jar` with a sample size larger than the size of the community.  
```{r, eval=FALSE}
...
This function is nested withhin the `sample()` function (you can see the code for `sample` by typing *sample* without brackets in the console). 
...
Also known as the species accumulation curve, the collector's curve plots the cummulative number of species (i.e., richness) as a function of sampling effort (i.e., number of individuals sampled, *N*).
...
Given that resources (time and money) are often limiting, what sampe size would you use for estimating richness of the source community?  
