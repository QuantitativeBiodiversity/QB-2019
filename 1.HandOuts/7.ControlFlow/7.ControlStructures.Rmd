...
Control structures allow you to control the flow of execution of a script (series of commands). Today, we will focus on the "for" loop, but other common control structures include: 
"if, else"
"while"
and "next"
You can find the documentation by typing `?Control` in the command line.
...
Compare iterating over a vector with it value and iterating by index
for (i in x) vd. for (i in seq(x))
**This chunk below seems to be going off topic with strings. That might need to be developed as an independent unit.**
...
Here we saw an example of how two control structures could be used together. If the first statement allowed you to check and  see if the proper library for handling strings was loaded on to our R packages and the second gave us an inventive new spelling for the word strawberry.
